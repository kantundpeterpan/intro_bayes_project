...
To model the effect of location and insurance type on vaccination coverage, we fit a logistic regression model:

$$
\text{logit}(\pi_t) = \mathbf{X}_t \boldsymbol{\beta}
$$

The likelihood is:
$$
y_t \sim \text{Binomial}(\pi_t, K_t), \quad \text{with} \quad \text{logit}(\pi_t) = \sum_{j=1}^p X_{tj} \cdot \beta_j
$$

where:
* \( y_t \) is the observed number of vaccinated children in group \( t \).
* \( K_t \) is the sample size in group \( t \).
* \( \pi_t \) is the vaccination probability in group \( t \).
* \( \mathbf{X}_t = (X_{t1}, X_{t2}, \ldots, X_{tp}) \) is the row vector of \( p \) covariates (intercept, geography, insurance type) for observation \( t \).
* \( \boldsymbol{\beta} = (\beta_{1}, \beta_{2}, \ldots, \beta_{p}) \) is the column vector of regression coefficients.
...
The design matrix we use creates dummy variables for each categorical variable with one reference level – Private for insurance and Georgia for geography (note that the reference level is changed for Question 8).

We assume non-informative priors for all parameters:

$$
\beta_j \sim \mathcal{N}(0, 100)
$$

which corresponds to `dnorm(0, 0.01)` in BUGS syntax.

### BUGS MODEL CODE

```{r, engine='BUGS', eval=FALSE}
model
{
  # Likelihood
  for (t in 1:T) {
...
We estimate one intercept parameter, \( \beta_1 \), and six slope parameters, \( \beta_2 \) through \( \beta_7 \), which quantify the effect of insurance status and geographic location on the log-odds of vaccination, relative to the reference group (children with private insurance in the state of Georgia):

* If \( \beta_j > 0 \), the log-odds of vaccination increase, corresponding to a higher probability of vaccination compared to the reference group.
* If \( \beta_j < 0 \), the log-odds of vaccination decrease, corresponding to a lower probability of vaccination compared to the reference group.
* If \( \beta_j = 0 \), there is no effect on the log-odds and thus no difference in the probability of vaccination relative to the reference group.

The log-odds are then transformed into estimated vaccination coverage \( \pi_t \) using the inverse logit function:

$$
\pi_t = \frac{\exp(\log(\text{odds}))}{1 + \exp(\log(\text{odds}))}
$$

The resulting value represents the probability that the children in the given group are vaccinated or, in other terms, the value represents the proportion of vaccinated children.

Based on our model estimates, children who have private insurance in the state of North Carolina have the highest vaccination probability of 94%. The lowest proportion of vaccinated children is in Wisconsin for uninsured children. The vaccination probability is 66.6%.

Additionally, we compare the estimates obtained from the logistic model with the analytical estimates given two conjugate priors: one noninformative Beta(1, 1) prior and one strongly informative prior with a vaccination coverage greater than 90%, namely Beta(150, 7).

Mostly, the derivations between the logit model estimates and the analytical estimates are small to moderate. However, in groups with small sample size — especially the case for uninsured children — the differences in estimates are noticeable. The smallest derivation in estimates differs by only 0.01 percentage points, while the largest differs by 25.2 percentage points.

Our results indicate that the choice of prior distributions can have a large impact on the estimates when the sample size is small.
...
### Convergence checks
Convergence of the MCMC chains was assessed using the same means as discussed in Question 3. None of the convergence diagnostics indicated any concerns, suggesting reliable sampling from the posterior distribution.
